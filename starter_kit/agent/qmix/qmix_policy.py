...
import ray
from agent.qmix.mixers import VDNMixer, QMixer
from agent.qmix.model import RNNModel, _get_size
...
class QMixLoss(nn.Module):
    ...
    def forward(self,
        ...
        targets = rewards.sum(-1, keepdim=True) + self.gamma * \
                  (1 - terminated.mean(-1, keepdim=True)) * target_max_qvals
...
class QMixTorchPolicy(Policy):
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        self.n_actions = action_space.spaces[0].n  # num of actions for one agent
    ...
    def learn_on_batch(self, samples):
        ...
        print('self.has_env_global_state:', self.has_env_global_state)
        obs_batch, action_mask, env_global_state = self._unpack_observation(
            samples[SampleBatch.CUR_OBS])
        # TODO
        print('env_global_state:', env_global_state)
        ...
        output_list, _, seq_lens = \
            chop_into_sequences(
                ...
                state_columns=[samples["state_in_{}".format(k)]
