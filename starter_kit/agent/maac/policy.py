...
import agent.maac
from agent.maac.model import MAACTorchModel
...
def make_model_and_action_dist(
    ...
    model_config = {**{key: config[key] for key in keys}}
    n_agents = len(obs_space.original_space.spaces)
   ...
   # print(action_space)
    ...
    policy.model = MAACTorchModel(
        ...
        num_outputs=3 * 4 #sum(action_space.nvec)
        # num_outputs=n_agents * action_space.spaces[0].n
    ).to(device)
    ...
    class MAACCOMTorchMultiCategorical(TorchMultiCategorical):
        def __init__(self, input, model):
            super().__init__(input, model, 3)
            # nbr_agents = len(obs_space.original_space.spaces)
            # nbr_actions = action_space.spaces[0].n
            # super().__init__(input, model, [nbr_actions] * nbr_agents)
...
def loss_critic(policy: Policy, model: ModelV2,
    ...
    reward = sample_batch[SampleBatch.REWARDS].unsqueeze(-1)
...
MAACTorchPolicy = build_torch_policy(
    ...
    get_default_config=lambda: agent.maac.trainer.DEFAULT_CONFIG,
