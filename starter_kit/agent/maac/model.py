...
class MAACTorchModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space: gym.spaces.Space,
        ...
        original_space = self.obs_space.original_space.spaces[0]
        print('original_space:', original_space)
        ...
        if not isinstance(self.true_obs_space, Box):
         ...
         '''
        self.nbr_agents = 4    #len(obs_space.original_space.spaces)
        self.nbr_actions = 3   # int(self.action_space.nvec[0])
        # self.nbr_actions = action_space.spaces[0].n  # num of actions for one agent
        # action_space = MultiDiscrete([self.nbr_actions] * self.nbr_agents)
        ...
        self.input_dim = 95 #self.true_obs_space.shape[0]
    ...
    def get_q_values(self, input_dict: Dict[str, TensorType]) -> TensorType:
        ...
        obs = self._unpack_observation(input_dict[SampleBatch.OBS])
        obs = torch.as_tensor(obs, dtype=torch.float, device=self.device)
        act = input_dict[SampleBatch.ACTIONS]
        act_one_hot = F.one_hot(act.to(torch.int64), self.nbr_actions).to(obs.dtype)
    ...
    def forward(self, input_dict: Dict[str, TensorType],
        ...
        B = obs.shape[0]
        obs = torch.split(obs, 1, dim=-2)  # split for each agent
        logits = [actor_(o.squeeze(-2)) for actor_, o in zip(self.actor, obs)]
        logits = torch.stack(logits, dim=-2)
        # print('logits:', logits.shape)
        #logits[action_mask == 0.0] = -float("inf")
    ...
    def _unpack_observation(self, obs_batch):
        ...
        distance_to_center = list(np.array(obs_batch['distance_to_center']).flat)
        goal_relative_pos = list(np.array(obs_batch['goal_relative_pos']).flat)
        heading_errors  = list(np.array(obs_batch['heading_errors']).flat)
        neighbor = list(np.array(obs_batch['neighbor']).flat)
        speed = list(np.array(obs_batch['speed']).flat)
        steering = list(np.array(obs_batch['steering']).flat)
        unpacked_obs = np.array(distance_to_center + goal_relative_pos + heading_errors + neighbor + speed + steering) 

        #action_mask = np.array([d['action_mask'].cpu().numpy() for d in obs_batch])
        #unpacked_obs = np.array([d['obs'].cpu().numpy() for d in obs_batch])  # shape [nbr_agents, B, input_dim]
        # print('unpacked_obs.shape:', unpacked_obs.shape)
        #action_mask = np.swapaxes(action_mask, 1, 0)
        obs = unpacked_obs.reshape([-1,self.nbr_agents, self.input_dim])
