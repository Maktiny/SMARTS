...
def action_adapter(model_action):
    ...
    brake = np.clip(model_action[0], -1, 0)
    steering = np.clip(model_action[1], -1 , 1)
...
OBSERVATION_SPACE = gym.spaces.Dict(
    {
        ...
        "distance_to_center": gym.spaces.Box(low=-1e10, high=1e10, shape=(1,),dtype=np.float32),
        # relative heading angle from 10 waypoints in 30 forehead waypoints
        "heading_errors": gym.spaces.Box(low=-1.0, high=1.0, shape=(30,),dtype=np.float32),
        ...
        "start_pos":gym.spaces.Box(low=-1e2, high=1e2, shape=(2,),dtype=np.float32),
        "goal_relative_pos": gym.spaces.Box(low=-1e2, high=1e2, shape=(2,),dtype=np.float32),
        # the ibformation of ego_vechiel's neighbor car, just 5
        "neighbor": gym.spaces.Box(low=-1e3, high=1e3, shape=(40,),dtype=np.float32),
        "waypoint": gym.spaces.Box(low=-1e3, high=1e3, shape=(62,),dtype=np.float32),
        "img_rgb": gym.spaces.Box(low=0.0, high=255.0, shape=(2,),dtype=np.float32), # 256 x 256 x 3 一张图片
    }
)
...
def cal_waypoint(env_obs):
    way_points=[]
    #print("WWWWWWWWWWWWWWWWW")
    #print(env_obs.waypoint_paths[0])
    for i in env_obs.waypoint_paths[0]:
        way_points.append(i.pos)
    waypoints = np.array(way_points)
    #print(waypoints.reshape((-1,)).shape)
    return waypoints.reshape((-1,))
    

def cal_img_rgb(env_obs: Observation):
...
def reward_adapter(observation, reward):
    ...
    ego_step_reward = 0.2 * min(speed_fraction, 1) * np.cos(angle_error)
    ...
    ego_reached_goal = 10.0 if ego_events.reached_goal else 0.0
