...
OBSERVATION_SPACE = gym.spaces.Dict(
    {
        ...
        "image_rgb": gym.spaces.Box(low=0.0, high=255.0, shape=(2,), dtype=np.float32),
    }
)
...
def cal_img_rgb(env_obs:Observation):
    img_rgb = env_obs.top_down_rgb
    return np.asarray(img_rgb)[1]
...
def reward_adapter(env_obs, env_reward):
    ...
    total_reward = np.sum([3.0 * env_reward])
